...
You answer: Origin your forked repository, while upstream is the main repository which you fork from.
...
You answer: I would stash the changes, use git checkout to change the branch to B, make the changes there and then move back to branch A using git checkout again.
...
You answer: git merge does not change the commits, while git rebase is used to change the commits, merge is quite the opposite of rebase if you think about it, since merge is quite literally used to preserver the commits, while rebase is used to change them. Example: you can use the git merge to merge changes in two branches, while you can use the git rebase to change up the commit history.
